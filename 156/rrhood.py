...
def make_character_index(text=text, characters=CHARACTERS):
    ...
    with StringIO(text) as in_file:
        for idx, line in enumerate(in_file):
            for character_names in characters:
                if isinstance(character_names, str):
                    character_names = (character_names,)
                hits = [
                    char_name.lower() in line.lower() for char_name in character_names
                ]
                if any(hits):
                    character_index[character_names[0].lower()].append(idx)

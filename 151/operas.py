...
def operas_both_at_premiere(guest, composer):
    ...
    guest_born, guest_died = _get_date(composers[guest].born), _get_date(
        composers[guest].died
    )
    composer_born, composer_died = _get_date(composers[composer].born), _get_date(
        composers[composer].died
    )
    return [
        opera.play
        for opera in composer_operas
        if (composer_born <= _get_date(opera.date) < composer_died)
        and (guest_born <= _get_date(opera.date) < guest_died)
    ]

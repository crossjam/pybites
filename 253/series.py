...
def series_simple_math(
    ...
    """
    return SERIES_FUNCTIONS[function](ser, number)
...
def complex_series_maths(
    ...
    return SERIES_FUNCTIONS[function](ser_01, ser_02)
...
def custom_series_function(ser: pd.Series, within: int) -> pd.core.series.Series:
    ...
    """
    quartiles = pd.qcut(ser, 4).cat.categories
    ...
    return ser[
        ser.apply(lambda v: any((qv - within <= v <= qv + within for qv in qvals)))
    ]
